...
class AdvancedLumpSumAnalyzer:
    ...
    def phase1_forecast_benchmarks(self):
        ...
        for bench in self.all_benchmarks:
            try:
                ...
                st.warning(f"⚠️ Error forecasting {bench}: {str(e)}")
                self.benchmark_cagr_forecasts[bench] = 0.0
    ...
    def phase2_forecast_funds(self):
        ...
        for fund in self.selected_funds:
            try:
                ...
                st.error(f"❌ Error forecasting {fund}: {str(e)}")
                self.cagr_forecasts[fund] = 0.0
    ...
    def plot_fund_vs_benchmark_forecast(self, fund_id):
        if fund_id not in self.cagr_forecasts or self.cagr_forecasts[fund_id] == 0.0:
            st.warning(f"⚠️ No forecast available for {fund_id}. Skipping plot.")
            return None

        try:
            nav_series = self.nav_data[fund_id].dropna()
            if len(nav_series) < 756:
                st.warning(f"⚠️ Insufficient history for {fund_id} to plot historical CAGR.")
                return None
            ...
            if fund_id not in self.model_selection_info:
                st.error(f"❌ No model info for {fund_id}")
                return None
            ...
            st.error(f"❌ Plot error for {fund_id}: {str(e)}")
            return None
...
def load_nav_data():
    if not os.path.exists("NAV_DATA.csv"):
        st.error("❌ NAV_DATA.csv not found in repository!")
        st.stop()
    return pd.read_csv("NAV_DATA.csv")
...
if len(selected_funds) < num_funds:
    st.warning("Please select all funds.")
    st.stop()
...
if st.sidebar.button("🚀 Run Analysis"):
    ...
    # Individual Fund Forecasts
    st.subheader("🔍 Individual Fund Forecasts")
    for fund in selected_funds:
        ...
        if fig:
            ...
            st.info(f"No forecast plot available for {fund}.")
...
st.sidebar.markdown("---")
st.sidebar.info("💡 Pro Tip: Select funds from different categories for better diversification!")