...
class AdvancedLumpSumAnalyzer:
    def __init__(self, nav_data):
        ...
        self.risk_parity_weights = None  # ✅ NEW
    ...
    def calculate_risk_parity_weights(self, covariance_matrix):
        ...
        return {fund: weights[i] for i, fund in enumerate(self.selected_funds)}
    ...
    def simple_markowitz_optimization(self):
        ...
        # Markowitz Optimization (Alpha/Risk Ratio)
        optimal_weights = self._optimize_portfolio(avg_returns, covariance, benchmark_returns)
        # Risk Parity Allocation
        risk_parity_weights = self.calculate_risk_parity_weights(covariance)

        self.optimal_weights = optimal_weights
        self.risk_parity_weights = risk_parity_weights  # ✅ STORED
        return optimal_weights, risk_parity_weights
    ...
    def calculate_lumpsum_returns(self, amount):
        ...
        # ✅ Risk Parity
        rp_growth = sum(self.risk_parity_weights[fund] * (self.cagr_forecasts[fund]/100) for fund in self.selected_funds)
        rp_projected = amount * (1 + rp_growth)

        self.lumpsum_results = {
            'equal': {'projected': eq_projected, 'growth': eq_growth},
            'markowitz': {'projected': opt_projected, 'growth': opt_growth},
            'risk_parity': {'projected': rp_projected, 'growth': rp_growth}  # ✅ STORED
        }
    ...
    def generate_summary_table(self):
        rows = []
        for fund in self.selected_funds:
            weight_m = self.optimal_weights[fund]
            weight_rp = self.risk_parity_weights[fund]  # ✅ ADDED
            cagr = self.cagr_forecasts[fund] / 100
            bench = self.benchmark_mapping[fund]
            bench_cagr = self.benchmark_cagr_forecasts.get(bench, 0.0) / 100
            alpha = cagr - bench_cagr
            allocated_amt_m = self.user_lumpsum_amount * weight_m
            allocated_amt_rp = self.user_lumpsum_amount * weight_rp  # ✅ ADDED
            projected_m = allocated_amt_m * (1 + cagr)
            projected_rp = allocated_amt_rp * (1 + cagr)  # ✅ ADDED
            rows.append({
                'Fund Name': self.fund_mapping[fund],
                'Markowitz Alloc': f"{weight_m:.1%}",
                'Risk Parity Alloc': f"{weight_rp:.1%}",  # ✅ ADDED
                '3Y CAGR': f"{cagr:+.1%}",
                ...
                'Markowitz Value': f"₹{projected_m:,.0f}",
                'Risk Parity Value': f"₹{projected_rp:,.0f}"  # ✅ ADDED
            })
        return pd.DataFrame(rows)


    def create_pie_chart(self, strategy='markowitz'):
        fig, ax = plt.subplots(figsize=(8, 8))
        if strategy == 'markowitz':
            weights = [self.optimal_weights[fund] for fund in self.selected_funds]
            title = 'Markowitz Allocation by Fund & Risk Profile'
        else:  # risk_parity
            weights = [self.risk_parity_weights[fund] for fund in self.selected_funds]
            title = 'Risk Parity Allocation (Equal Risk Contribution)'

        labels = [f"{self.fund_mapping[fund]}\n{w:.0%} [{self.risk_levels[fund]}]" for fund, w in zip(self.selected_funds, weights)]
        colors = plt.cm.Set3.colors[:len(weights)] if strategy == 'markowitz' else plt.cm.Pastel1.colors[:len(weights)]
    ...
    def create_bar_chart(self):
        ...
        strategies = ['Equal Allocation', 'Markowitz', 'Risk Parity']
        values = [
            self.lumpsum_results['equal']['projected'],
            self.lumpsum_results['markowitz']['projected'],
            self.lumpsum_results['risk_parity']['projected']
        ]
        colors = ['#87CEEB', '#F4A460', '#98FB98']
        bars = ax.bar(strategies, values, color=colors, alpha=0.85, edgecolor='black', linewidth=1.5)
    ...
    def export_to_pdf(self):
        ...
        with PdfPages(buf) as pdf:
            ...
            table.scale(1.3, 2)
            ax.set_title('Investment Allocation Summary: Markowitz vs Risk Parity', fontsize=14, fontweight='bold', pad=20)
            pdf.savefig(fig)
            plt.close(fig)

            # Pie Charts
            fig = self.create_pie_chart('markowitz')
            pdf.savefig(fig)
            plt.close(fig)

            fig = self.create_pie_chart('risk_parity')
            pdf.savefig(fig)
            plt.close(fig)
...
st.markdown("### AI-Powered 3-Year CAGR Forecasting vs Benchmarks (Markowitz & Risk Parity)")
...
if st.sidebar.button("🚀 Run Analysis"):
    ...
    with st.spinner("⏳ Phase 2: Optimizing Portfolio (Markowitz & Risk Parity)..."):
    ...
    # Summary Table
    st.subheader("📋 Allocation & Performance Summary (Markowitz vs Risk Parity)")
    ...
    with col1:
        st.subheader("🥧 Markowitz Allocation")
        pie_fig_m = analyzer.create_pie_chart('markowitz')
        st.pyplot(pie_fig_m)

    with col2:
        st.subheader("⚖️ Risk Parity Allocation")
        pie_fig_rp = analyzer.create_pie_chart('risk_parity')
        st.pyplot(pie_fig_rp)

    # Growth Comparison
    st.subheader("💰 Projected Growth: Equal vs Markowitz vs Risk Parity")
    ...
    st.download_button(
        ...
        help="✅ Includes summary table, both pie charts, bar chart, and all forecast plots."
    )

st.sidebar.markdown("---")
st.sidebar.info("💡 Pro Tip: Combine Large, Mid, Small Cap funds — Risk Parity naturally balances risk across categories for smoother returns!")