...
# ========== ANALYZER CLASS (Optimized for Caching) ==========
class AdvancedLumpSumAnalyzer:
    ...
    def plot_fund_vs_benchmark_forecast(self, fund_id):
        if fund_id not in self.cagr_forecasts or self.cagr_forecasts[fund_id] == 0.0:
            return None

        try:
            nav_series = self.nav_data[fund_id].dropna()
            if len(nav_series) < 756:
                return None

            y = (nav_series / nav_series.shift(756)) ** (1/3) - 1
            hist_cagr = y.dropna()

            if fund_id not in self.model_selection_info:
                return None
...
@st.cache_resource
def load_analyzer():
    if not os.path.exists("NAV_DATA.csv"):
        st.error("❌ NAV_DATA.csv not found!")
        st.stop()
    df = pd.read_csv("NAV_DATA.csv")
    analyzer = AdvancedLumpSumAnalyzer(df)
    analyzer.load_and_validate_data()
    # ✅ Pre-compute benchmarks ONCE (they never change)
    analyzer.phase1_forecast_benchmarks()
    return analyzer

analyzer = load_analyzer()

# ✅ CACHE ANALYSIS BY FUND SELECTION (key = sorted fund list + amount)
@st.cache_data
def run_analysis(selected_funds, amount):
    # Create fresh copy to avoid state pollution
    import copy
    local_analyzer = copy.deepcopy(analyzer)
    local_analyzer.selected_funds = selected_funds

    # Run phases
    local_analyzer.simple_markowitz_optimization()
    local_analyzer._calculate_risk_levels()
    local_analyzer.phase2_forecast_funds()
    local_analyzer.calculate_lumpsum_returns(amount)

    # Serialize results for caching
    return {
        'optimal_weights': local_analyzer.optimal_weights,
        'cagr_forecasts': local_analyzer.cagr_forecasts,
        'risk_levels': local_analyzer.risk_levels,
        'model_selection_info': local_analyzer.model_selection_info,
        'lumpsum_results': local_analyzer.lumpsum_results,
        'benchmark_cagr_forecasts': local_analyzer.benchmark_cagr_forecasts,
        'benchmark_forecast_details': local_analyzer.benchmark_forecast_details
    }
...
for i in range(num_funds):
    ...
    category = st.sidebar.selectbox(f"Category", categories, key=f"cat_{i}")
    ...
    if fund_names:
        ...
        st.sidebar.warning(f"No funds in {category}")

if len(set(selected_funds)) < num_funds:
    st.warning("⚠️ Please select unique funds.")
    st.stop()

investment_amount = st.sidebar.number_input("Investment Amount (₹)", min_value=1000, value=100000, step=1000)

# ✅ RUN ANALYSIS ONLY IF FUNDS OR AMOUNT CHANGE
fund_key = tuple(sorted(selected_funds))  # Make key hashable
results = run_analysis(fund_key, investment_amount)

# ✅ Inject cached results back into analyzer for plotting
analyzer.selected_funds = selected_funds
analyzer.optimal_weights = results['optimal_weights']
analyzer.cagr_forecasts = results['cagr_forecasts']
analyzer.risk_levels = results['risk_levels']
analyzer.model_selection_info = results['model_selection_info']
analyzer.lumpsum_results = results['lumpsum_results']
analyzer.benchmark_cagr_forecasts = results['benchmark_cagr_forecasts']
analyzer.benchmark_forecast_details = results['benchmark_forecast_details']

if st.sidebar.button("🚀 Generate Report"):
    st.success("✅ Report Ready (Cached for Speed!)")

    st.header("📈 Analysis Results")
    st.subheader("📋 Investment Allocation Summary")
    ...
    with col1:
        ...
        st.pyplot(analyzer.create_pie_chart())
    with col2:
        st.subheader("📊 Projected Value")
        st.pyplot(analyzer.create_bar_chart())

    st.subheader("🔍 Fund Forecasts")
    for fund in selected_funds:
        st.markdown(f"### {analyzer.fund_mapping[fund]} ({fund})")
    ...
    st.download_button(
        "📄 Download PDF",
        data=pdf_buffer,
        file_name="Lumpsum_Analysis.pdf",
        mime="application/pdf"
    )

st.sidebar.markdown("---")
st.sidebar.caption("✅ Benchmarks pre-computed | ✅ Analysis cached | ⚡ Blazing fast after first run")